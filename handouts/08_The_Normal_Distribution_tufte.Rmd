...
author: "Math 530-630, Note Set #8"
header-includes:
    \usepackage{graphicx}
    \date{}
    \renewcommand\plaintitle{Math 530-630, (8) The Normal Distribution}
...
\begin{marginfigure}
\includegraphics[width=25mm]{Gauss.jpg}  
\caption{Johann Carl Friedrich Gauss, 1777-1855 (source: Wikipedia)}
\end{marginfigure}


\newthought{The univariate normal (or Gaussian) distribution} is a very common continuous probability distribution. When the probability density of a random variable, $X$, taking on any value, $x$, is given by the function:

$$f(X = x) = \frac{1}{\sigma \sqrt{2\pi}} e^{-\frac{(x-\mu)^2}{2 \sigma^2}}$$ 

then $X$ is said to be a normally distributed variable.^[or more formally: $X \sim \mathcal{N}(\mu, \sigma^2)$] 

```{r, echo=FALSE, fig.height= 2, fig.align='center', fig.cap = "f(X) is the probability density function for a random variable, X, with a normal distribution"}
library(ggplot2)
library(grid)
norm <- ggplot(data.frame(x = c(-4, 4)), aes(x)) + 
  stat_function(fun = dnorm, lwd = .25, arg = list(mean = 0, sd = 1)) + 
  geom_segment(x = 0, xend = 0, y = 0, yend = .41, col = "dodgerblue", lwd = .25) +
  ...
  geom_rect(aes(x = NULL, y = NULL, xmin = -1, xmax = 1, ymin = 0, ymax = .4), fill = "dodgerblue", alpha = .1) +
  coord_cartesian(ylim = c(-.035, .42))

norm + 
  annotate('text',x = 0, y = -0.02, label = "mu",parse = TRUE, size = 3, family="serif")  + 
  annotate("text", x = -1, y = -0.02, label = "-sigma", parse = TRUE, size = 3, family="serif") + 
  annotate("text", x = 1, y = -0.02, label = "+sigma", parse = TRUE, size = 3, family="serif")
```  
...
This is a whole (infinite) family of distributions with $\mu$ any finite value and $\sigma^2 > 0$. This is a probability density function (p.d.f.) because $f(x) \geq 0$ and  
...
2. Assumption of normality is important for mathematical reasons and underlies much of inferential statistics. For example, many uses of inferential statistics **assume** that $\mu$ and $\sigma^2$ are **independent** -- but this is true only for the normal distribution. ^[For example, recall the Binomial Distribution where $$E(X) = \mu = np$$ $$Var(X) = \sigma^2 = npq$$
Thus, $Var(X) = qE(X)$, i.e. $\mu$ and $\sigma^2$ are not independent.]  
...
norm_lta + 
  ...
  annotate("text", x = -1, y = -0.02, label = "-sigma", parse = TRUE, size = 3, family="serif") + 
  annotate("text", x = 1, y = -0.02, label = "+sigma", parse = TRUE, size = 3, family="serif") +
  annotate("text", x = 2, y = -0.02, label = "a", size = 3, family="serif")
```  

\marginnote{Figure 3 shows $P(x \leq a) = F(a)$ as the area under the curve in green.} 
...
```{r, echo=FALSE, fig.height= 2, fig.align='center', fig.cap = "The normal cdf"}
cdf <- ggplot(df, aes(x)) +
  stat_function(fun = pnorm, lwd = .25, arg = list(mean = 0, sd = 1)) +
  annotate("rect", xmin = -1, xmax = 1, ymin = 0, ymax = 1, alpha = 0.1, fill = "dodgerblue") +
  theme(axis.title = element_blank(), 
         panel.grid = element_blank(), 
         panel.background = element_blank(), 
         axis.text = element_blank(), 
         axis.ticks = element_blank()) +
  geom_hline(aes(yintercept = 0), lwd = .25) +
  geom_segment(aes(x=0,y=0,xend=0,yend=1), lwd = .25, colour = "dodgerblue") +
  geom_segment(aes(x=2,y=0,xend=2,yend=1), colour = "seagreen", lwd = .35, lty = "dotted") 
   
cdf + 
  annotate("text", x = -.25, y = 0.5, label = "0.5", size = 2, family = "serif") +
  annotate("text", x = -.25, y = 1.0, label = "1.0", size = 2, family = "serif") +
  annotate("text", x = 0, y = -.05, label = "mu", parse = TRUE, size = 3, family = "serif") +
  annotate("text", x = 2, y = -.05, label = "a", size = 3, family = "serif")
```
...
\marginnote{Figure 4 shows $P(x > a) = F(a)$ as the area under the curve in green.} 
...
$z = \displaystyle\frac{(X - \mu)}{\sigma}$ is a **transformation** of the random variable _x_. 

\newthought{We start with the cumulative distribution function}  

$$F(a) = \displaystyle\frac{1}{\sigma \sqrt{2\pi}} \displaystyle\int_{-\infty}^{a} e^{-\frac{(x-\mu)^2}{2\sigma^2}} dx$$  

We recognize the exponent of _e_ as $\frac{-z^2}{2}$. So we have  
...
1. $p(z \geq 2) = F(z) = 0.9772499$  

2. $p(z > 2) = 1 - p(z > 2) = 1 - 0.9772499$   
$= 0.0227501$  

3. $p(z \leq -1) = 1 - f(1) = 1 - 0.8413447$   
$= 0.1586553$  
...
4. What is the 99th percentile, i.e. what z-score yields this value $F(z) = 0.99$? $z \approx 2.33$.  

5. What SAT score is that?  
$z = 2.33 = \displaystyle\frac{x - 500}{100} = 733$  


```{r echo = FALSE, fig.align = 'center', fig.height= 2, fig.cap = "The (hypothetical) normal pdf of SAT scores"}
x <- pretty(c(100, 900), 1000)
y <- dnorm(x, 500, 100)
df <- data.frame(x = x, y = y)
norm_sat <- ggplot(df, aes(x)) + 
  stat_function(fun = dnorm, lwd = .25, arg = list(mean = 500, sd = 100)) + 
  annotate("rect", xmin = 400, xmax = 600, ymin = 0, ymax = .0042, alpha = 0.1, fill = "dodgerblue") +
  geom_ribbon(data = subset(df, x > 733), aes(ymin = 0, ymax = y), fill = "seagreen", alpha  = .25) +
  geom_segment(x = 500, xend = 500, y = 0, yend = .0044, col = "dodgerblue", lwd = .25) +
  geom_segment(x = 733, xend = 733, y = 0, yend = .0044, col = "seagreen", lwd = .35, lty = "dotted") +
  geom_hline(yintercept = 0, lwd = .25) +
  theme(axis.title = element_blank(), 
        panel.grid = element_blank(), 
        panel.background = element_blank(), 
...
norm_sat + 
  annotate('text',x = 500, y = -.0002, label = "500",parse = TRUE, size = 3, family="serif")  + 
  annotate("text", x = 400, y = -.0002, label = "400", parse = TRUE, size = 3, family="serif") + 
  annotate("text", x = 600, y = -.0002, label = "600", parse = TRUE, size = 3, family="serif") +
  annotate("text", x = 733, y = -.0002, label = "733", size = 3, family="serif")
```  
...
$$\qquad \qquad p(-1.96 \leq z_{\mu} \leq 1.96) = 0.95$$



```{r echo = FALSE, fig.align = 'center', fig.height= 2, fig.cap = "A confidence interval"}
library(grid)
x <- pretty(c(-4,4), 1000)
y <- dnorm(x, 0, 1)
df <- data.frame(x = x, y = y)
norm_ci <- ggplot(df, aes(x)) + 
  stat_function(fun = dnorm, lwd = .25, arg = list(mean = 0, sd = 1)) + 
  annotate("rect", xmin = -1, xmax = 1, ymin = 0, ymax = .4, alpha = 0.1, fill = "dodgerblue") +
  annotate("rect", xmin = -1.96, xmax = 1.96, ymin = 0, ymax = .4, alpha = 0.25, fill = "moccasin") +
  geom_ribbon(data = subset(df, x > 1.96), aes(ymin = 0, ymax = y), fill = "seagreen", alpha  = .25) +
  geom_ribbon(data = subset(df, x < -1.96), aes(ymin = 0, ymax = y), fill = "seagreen", alpha  = .25) +
  geom_segment(x = 0, xend = 0, y = 0, yend = .41, col = "dodgerblue", lwd = .25) +
  geom_segment(x = 1.96, xend = 1.96, y = 0, yend = .41, col = "seagreen", lwd = .35, lty = "dotted") +
  geom_segment(x = -1.96, xend = -1.96, y = 0, yend = .41, col = "seagreen", lwd = .35, lty = "dotted") +
  geom_hline(yintercept = 0, lwd = .25) +
  theme(axis.title = element_blank(), 
        panel.grid = element_blank(), 
        ...
        axis.text = element_blank(), 
        axis.ticks = element_blank())  +
  coord_cartesian(ylim = c(-.035, .48))

norm_ci + 
  annotate('text',x = 0, y = -0.02, label = "mu",parse = TRUE, size = 3, family="serif")  + 
  annotate("text", x = -1, y = -0.02, label = "-sigma", parse = TRUE, size = 3, family="serif") + 
  annotate("text", x = 1, y = -0.02, label = "+sigma", parse = TRUE, size = 3, family="serif") +
  annotate("text", x = -3, y = 0.05, label = "p = .025", size = 2.5, family="serif")+
  annotate("text", x = 3, y = 0.05, label = "p = .025", size = 2.5, family="serif") +
  annotate("text", x = 0, y = .43, label = "p = .95", size = 2.5, family="serif") +
  geom_segment(aes(x=-.5,y=0.43,xend=-1.95,yend=0.43),
               arrow=arrow(length = unit(0.1,"cm")), lwd = .1) +
  geom_segment(aes(x=.5,y=0.43,xend=1.95,yend=0.43),
               arrow=arrow(length = unit(0.1,"cm")), lwd = .1)
```  


^[The area in yellow shows you that: $p(\mu \text{ somewhere in here}) = 0.95$]
and here

$$p = 2 \times F(1.96) - 1 = 0.95$$  
...
```{r echo = FALSE, fig.margin = TRUE, fig.cap = "A confidence interval for z scores"}
library(grid)
z_ci <- ggplot(df, aes(x)) + 
  stat_function(fun = dnorm, lwd = .25, arg = list(mean = 0, sd = 1)) + 
  annotate("rect", xmin = -1, xmax = 1, ymin = 0, ymax = .4, alpha = 0.1, fill = "dodgerblue") +
  annotate("rect", xmin = -1.96, xmax = 1.96, ymin = 0, ymax = .4, alpha = 0.25, fill = "moccasin") +
  geom_ribbon(data = subset(df, x > 1.96), aes(ymin = 0, ymax = y), fill = "seagreen", alpha  = .25) +
  ...
  theme(axis.title = element_blank(), 
        ...
        axis.text = element_blank(), 
        axis.ticks = element_blank())  +
  coord_cartesian(ylim = c(-.045, .48))

z_ci + 
  annotate('text',x = 0, y = -0.02, label = "0",parse = TRUE, size = 4.5, family="serif")  + 
  annotate("text", x = -1, y = -0.02, label = "-1.96", parse = TRUE, size = 4.5, family="serif") + 
  annotate("text", x = 1, y = -0.02, label = "+1.96", parse = TRUE, size = 4.5, family="serif") +
  annotate("text", x = -3.5, y = 0.05, label = "p = .025", size = 4.5, family="serif")+
  annotate("text", x = 3.5, y = 0.05, label = "p = .025", size = 4.5, family="serif") +
  annotate("text", x = 0, y = .43, label = "p = .95", size = 4.5, family="serif") +
  geom_segment(aes(x=-.75,y=0.43,xend=-1.95,yend=0.43),
               arrow=arrow(length = unit(0.15,"cm")), lwd = .05) +
  geom_segment(aes(x=.75,y=0.43,xend=1.95,yend=0.43),
               arrow=arrow(length = unit(0.15,"cm")), lwd = .05)
```  
